...
class Trainer:
    ...
    def fit(self, X_train, y_train, X_val, y_val, label_encoder=None):
        ...
        print(f"  LR       : {self.cfg['training']['learning_rate']}  |  Grad clip: {self.clip_grad}")
        print(f"{'='*55}\n")
        ...
        save_path   = os.path.join(results_dir, f"confusion_matrix_{arch.lower()}.png")

        evaluate(y_val, y_pred, label_encoder=label_encoder, model_name=arch)
        plot_confusion_matrix(y_val, y_pred, label_encoder=label_encoder,
                              title=f"Confusion Matrix — {arch}", save_path=save_path)
    ...
    def _save_checkpoint(self):
        ...
        path = os.path.join(folder, f"{self.cfg['model']['architecture']}.pt")
